...
function Header() {
const { dispatch, cartItem} = useContext(CartContext)
...
function handleMobileNav (){
  ...
  return (
    ...
    <span className="bg-[#EFF0D1] w-full inline-block py-5 text-center text-red-900">Black Friday's discount - Take advantage of them!</span>
          <div className="lg:flex justify-between md:flex lg:mx-20 mx-9 px-9 py-5 hidden md:px-0 md:mx-3">
              <Link to='/home' className="flex text-xl md:text-1xl text-white"><p className="font-bold">Cozy</p> Hub</Link>
                ...
                <span className="flex justify-between items-center space-x-5">
                    <select className="bg-red-900 outline-none text-white" name="" id="">
                    ...
                    <span className="flex justify-between items-center text-white space-x-5">
                        ...
                        <FaRegUser />
                        <Link><p onClick={handleCart} className="text-sm border-white border px-[6px] lg:px-[12px] py-2 rounded-full">Cart {cartItem}</p></Link>
                    </span>
                </span>
        </div>
    </div>
    ...
    <Link onClick={handleMobileNav} className="lg:hidden md:hidden visible fixed right-0 top-20 mx-4 z-50 cursor-pointer '">
  ...
  {/* <CiMenuBurger className='size-6 lg:hidden md:hidden visible fixed right-0 top-10 z-50 bg-white p-4 mx-3  text-red-900 rounded-full cursor-pointer' onClick={handleMobileNav}/> */}
    ...
    <div className={`bg-red-900 fixed h-full right-0 top-0 z-50 ${isVisible ? 'toggle' : 'untoggle'}`}>
        <div className=" flex gap-9 mx-3 px-3 lg:px-10 py-14 space-x-7">
            <span className="grid text-white text-lg gap-3">
              <Link to='/' className="flex text-xl text-1xl text-white"><p className="font-bold">Cozy</p> Hub</Link>
                   <NavLink to='/shop'>SHOP</NavLink> 
                   <NavLink to='/showroom'>SHOWROOM</NavLink> 
                   <NavLink to='/brands'>BRANDS</NavLink> 
                   {/* <NavLink to='/users'>USER</NavLink>  */}
              ...
              <Link to="#">
              <p onClick={handleMobileNav} className='text-2xl border border-white text-white px-2.5 py-[1px] rounded-full border-spacing-1 cursor-pointer'>&times;</p>
            </Link>
        </div>
                    <span className="flex items-center text-white mx-6 lg:px-9 px-2 space-x-5">
                        <FaRegUser />
                        <Link><p onClick={handleCart} className="text-sm border-white border px-3 py-2 rounded-full">Cart {cartItem}</p></Link>
                    </span>
    </div>
