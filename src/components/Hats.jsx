...
function Hats() {
    const { dispatch, cartProduct, setCartProduct } = useContext(CartContext);
    ...
    const HatCategory = items.filter((item) => item.title === 'Hats')
                ...
                <div key={i}  className='grid lg:grid-cols-5 items-baseline gap-y-7 sm:grid-cols-3'>
                  {item.items.map((item, i) => (
                    <div key={i} className='py-4'>
                      ...
                      <button onClick={() => handleCart(item)} className=''>Add to cart </button>
